...
public abstract class AbstractBattleField implements Poolable {
	...
	private Shoot lastShootPlayed = null;
	...
	public Shoot getLastMovePlayed() {
		return lastShootPlayed;
	}

	protected void clear() {
		currentPlayer = null;
		lastShootPlayed = null;
		gameOver = false;
	}
	 ...
	 * Calls {@link AbstractBoard#makeMove(Shoot, List, boolean) makeMove} method on {@link AbstractBattleField}. 
	 ...
	 * 	</ul>
	 */
	public int playMove(Shoot move, List<Position> changes, boolean searching) {
		int result = makeMove(move, changes, searching);
		if (result != 0) {
			lastShootPlayed = move;
		}
		nextPlayer();
		return result;
	}
	...
	public void copy(AbstractBattleField board) {
		if (board instanceof AbstractBattleField) {
			this.gameContext = ((AbstractBattleField) board).gameContext;
			this.currentPlayer = ((AbstractBattleField) board).currentPlayer;
			this.lastShootPlayed = ((AbstractBattleField) board).lastShootPlayed;
			this.gameOver = ((AbstractBattleField) board).gameOver;
		}
	}
	 ...
	 * null and is a {@link AbstractBattleField} of the same subclass as this one, and represents the same board (it has
	...
	public boolean equals(Object obj) {
		...
		final AbstractBattleField other = (AbstractBattleField) obj;
	 ...
	 * to generate {@link Shoot} instances.
	...
	public abstract List<Shoot> getValidMoves(ShootRanker moveRanker, int depth);
	 ...
	 * 	</ul>
	 */
	public abstract int makeMove(Shoot move, List<Position> changes, boolean searching);
