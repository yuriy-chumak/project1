...
import com.wata.battleships.AbstractBattleField;
import com.wata.battleships.Shoot;
import com.wata.battleships.ShootList;
import com.wata.battleships.ShootRanker;
import com.wata.battleships.Player;
import com.wata.battleships.Position;

/**
 * BattleField. It is is a 10x10 square board; each square on the board may be empty, or may contain a black piece or a
 ...
 * 
 */
public class BattleField extends AbstractBattleField implements CompareBoards {
	...
	public void initialise() {
		...
		setCurrentPlayer(BattleshipsPlayer.getInstance(Colour.BLACK));
	}
	 ...
	 */
	public CompareBoards compare(BattleField reversiBoard, BattleField differenceBoard) {
	...
	public boolean isValidMove(int x, int y) {
		return isValidMove(x, y, ((BattleshipsPlayer) getCurrentPlayer()).getColour());
	}
	...
	private int traverseLine(int x, int y, int colour, int dx, int dy, boolean fakeIt, List<Position> flipped) {
		...
		while (ox >= 1 && ox <= X_DIMENSION && oy >= 1 && oy <= Y_DIMENSION) {
			...
			if (colr != colour) {
				flips++;
				if (flipped != null) {
					flipped.add(new BattleshipsPosition(ox, oy));
				}
				if (!fakeIt) {
					flipPiece(ox, oy);
				}
	...
	public String toString() {
		...
		BattleshipsPlayer player = (BattleshipsPlayer) getCurrentPlayer();
		for (int y = 1; y <= Y_DIMENSION; y++) {
			...
			for (int x = 1; x <= X_DIMENSION; x++) {
				...
				if (squareColour != Colour.NONE) {
					buffer.append(BattleshipsPlayer.getInstance(squareColour).getSymbol());
	...
	public void fromString(String boardAsString) {
		...
		for (int i = 1; i < lines.length; i++) {
			for (int j = 2; j < lines[i].length(); j += 2) {
				...
				if (BattleshipsPlayer.getInstance(Colour.BLACK).getSymbol() == piece.charAt(0)) {
					setPiece(x, y, Colour.BLACK);
				} else if (BattleshipsPlayer.getInstance(Colour.WHITE).getSymbol() == piece.charAt(0)) { 
	...
	@Override
	public void copy(AbstractBattleField board) {
		if (! (board instanceof BattleField)) {
			throw new IllegalArgumentException("Cannot copy " + board.getClass().getName() + " to " + getClass().getName());
		}
		super.copy(board);
		BattleField reversiBoard = (BattleField) board;
		for (int i = 0; i <= 7; i++) {
	...
	public boolean canMove() {
		int colour = ((BattleshipsPlayer) getCurrentPlayer()).getColour();
	...
	public int makeMove(Shoot move, List<Position> changes, boolean searching) {
		int x = ((BattleshipsShoot) move).getPosition().getX();
		int y = ((BattleshipsShoot) move).getPosition().getY();
		int colour = ((BattleshipsPlayer) getCurrentPlayer()).getColour();
		int flips = 0;
		long l = adjacents[2 - colour][y - 1] >> (8 * (x - 1));
	...
	@Override
	public List<Shoot> getValidMoves(ShootRanker moveRanker, int depth) {
		int colour = ((BattleshipsPlayer) getCurrentPlayer()).getColour();
		if (gameOver || validCounts[colour - 1] == 0) {
			return Collections.emptyList();
		} else {
			ShootList moves = gameContext.createMoveList(INITIAL_MOVE_LIST_CAPACITY);
			for (int y = 0; y <= 7; y++) {
				if (adjacents[2 - colour][y] != 0) {
					for (int x = 0; x <= 7; x++) {
						if (((adjacents[2 - colour][y] >> (8 * (x))) & 255) != 0) {
							if (isValidMove(x + 1, y + 1, colour)) {
								...
								BattleshipsShoot reversiMove = ((BattleshipsShootFactory) gameContext.getMoveFactory()).createMove(x + 1, y + 1);
	...
	public int countValidMoves(Player player) {
		// see if we've already calculated it
		int colour = ((BattleshipsPlayer) player).getColour();
